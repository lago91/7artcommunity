...
class news extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('usuario_idusuario, votos, status', 'numerical', 'integerOnly'=>true),
			array('titulo', 'length', 'max'=>45),
			array('descripcion', 'length', 'max'=>1000),
			array('fechaPublicacion', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idnoticia, titulo, descripcion, fechaPublicacion, usuario_idusuario, votos, status', 'safe', 'on'=>'search'),
		);
	}
